...
function TablePagination({
  ...
  const controlsDisabled =
    disabled || typeof onPageChange !== 'function'
  ...
  return (
    <nav
      ...
      className={[
        'flex flex-wrap items-center justify-between gap-3 border-t border-border-default bg-surface-card px-4 py-3',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
    >
      <p className="text-role-helper text-text-secondary">
        {normalizedTotalItems === 0
          ? 'No results'
          : `Showing ${rangeStart}–${rangeEnd} of ${normalizedTotalItems}`}
      </p>
